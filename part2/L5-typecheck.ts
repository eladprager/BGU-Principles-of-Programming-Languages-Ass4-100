...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isLetValuesExp, LetValuesExp, CExp, ValuesBindings } from "./L5-ast";
...
const typeOfValues = (oprands: CExp[], tenv: TEnv): Result<TExp> => {
    const typeOfOpRands = mapResult((rand: CExp) => typeofExp(rand, tenv), oprands);
    const unparsedTypesOfOpRands = bind(typeOfOpRands, unparsedOpRands);
    const joinedStrs = bind(unparsedTypesOfOpRands, joinStrs);
    return bind(joinedStrs, parseTE);
}


// functions for typeofletvalues
const unparsedOpRands = (texparr: TExp[]): Result<string[]> => 
        mapResult((t: TExp) => unparseTExp(t), texparr); 

const joinStrs = (strarr: string[]) => makeOk(`(${join(" * ", strarr)})`)

const TwodTO1dVardecls = (vardecls: VarDecl[][]): string[] => 
    chain((x) => map((v: VarDecl) => v.var, x), vardecls);   
            
const TwodTO1dValues = (values: TExp[][]): TExp[] => 
    chain((x) => x, values);
// end of dunctions for typeofvalues


export const typeofLetvalues = (letv: LetValuesExp, tenv: TEnv): Result<TExp> => {
    const ArrOfValues = map((vbinds: ValuesBindings) => vbinds.value, letv.bindings);
    const valuesTExps = mapResult((value: CExp) => typeofExp(value, tenv), ArrOfValues);
    const AllValuesTuplesTEs = bind(valuesTExps, (texparr: TExp[]): Result<TExp[][]> => 
            mapResult((t: TExp) => isTupleTExp(t) ? isEmptyTupleTExp(t) ? makeOk([]) : makeOk(t.TEs) : 
            makeFailure(`Not a tuple!`), texparr));

    const ArrOfVariables = map((b: ValuesBindings) => b.vardecls, letv.bindings);
    const variableTEs = map((vars) => map((v) => v.texp, vars), ArrOfVariables);
    
    const checkEqualSize = bind(AllValuesTuplesTEs, (TEvalues: TExp[][]) => 
        bind(mapResult((variables: TExp[]) => {
            const matchValue = TEvalues[variableTEs.indexOf(variables)];
            return matchValue.length === variables.length ? makeOk(true) :
            makeFailure(`Wrong size!`);
        }, variableTEs), _ => makeOk(TEvalues)));
    
    const Cons = (varTEs: TExp[], valTEs: TExp[]) =>
        zipWithResult((varTE, valTE) => checkEqualType(varTE, valTE, letv),
            varTEs, valTEs);
    
    const allCons = bind(checkEqualSize, 
        (valuesTEs: TExp[][]) => 
            mapResult((varTEs: TExp[]) => 
                Cons(varTEs, valuesTEs[variableTEs.indexOf(varTEs)]), variableTEs));
    

    return bind(allCons, _ => typeofExps(letv.body, makeExtendTEnv(TwodTO1dVardecls(ArrOfVariables), TwodTO1dValues(variableTEs), tenv)));
