...
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, isLetValuesExp, LetValuesExp, ValuesBindings } from './L5-ast';
...
// functions for evalletvalues
const isEqualLength = (arr1: VarDecl[], arr2: SExpValue[]): boolean => arr2.length === arr1.length;

const checkEqualSize = (vars: VarDecl[][], vals: SExpValue[][]): Result<boolean[]> => 
        mapResult((vardecls: VarDecl[]) =>
        isEqualLength(vardecls, vals[vars.indexOf(vardecls)]) ?
                    makeOk(true) : 
                    makeFailure(`Size is not the same`)
                    , vars)

const TwodTO1dVardecls = (vardecls: VarDecl[][]): string[] => 
    chain((x) => map((v: VarDecl) => v.var, x), vardecls);   
            
const TwodTO1dValues = (values: SExpValue[][]): SExpValue[] => 
    chain((x) => x, values);
// end of functions for evalletvalues


const evalLetValues = (letvexp: LetValuesExp, env: Env): Result<Value> => {
    const takeTuple = (v: SExpValue): Result<SExpValue[]> => 
         isTuple(v) ? makeOk(v.vals) : makeFailure(`No tuple value`)
    const ArrOfValues1 = map((b: ValuesBindings) => b.value, letvexp.bindings)
    const evalValues = mapResult((v: CExp) => applicativeEval(v, env), ArrOfValues1);
    const tupleValues = bind(evalValues, (vals: SExpValue[]) => mapResult(takeTuple, vals))
    
    const ArrOfValues2 = makeOk(map((b: ValuesBindings) => b.vardecls, letvexp.bindings));

    return safe2((vardecls: VarDecl[][], values: SExpValue[][]): Result<Value> => 
            bind(checkEqualSize(vardecls, values), _ =>
            evalSequence(letvexp.body, makeExtEnv(TwodTO1dVardecls(vardecls), TwodTO1dValues(values), env))))
        (ArrOfValues2, tupleValues);
}
