...
export const expComponents = (e: Exp): CExp[] =>
    ...
    isSetExp(e) ? [e.val] :
    isDefineExp(e) ? [e.val] :
    isLetValuesExp(e) ? [...e.body, ...map((b) => b.value, e.bindings)] :
...
export interface ValuesBindings {tag: "ValuesBindings"; vardecls: VarDecl[]; value: CExp; }
export const makeValuesBinding = (vars: VarDecl[], val: CExp): ValuesBindings =>
    ({tag: "ValuesBindings", vardecls: vars, value: val});
export const isValuesBinding = (x: any): x is ValuesBindings => x.tag === "ValuesBindings";

export interface LetValuesExp {tag: "LetValuesExp"; bindings: ValuesBindings[]; body: CExp[]; }
export const makeLetValuesExp = (bindings: ValuesBindings[], body: CExp[]): LetValuesExp =>
...
const parseValuesBindings = (bindings: [Sexp[], Sexp][]): Result<ValuesBindings[]> =>
    safe2((vardecls: VarDecl[][], values: CExp[]) => makeOk(zipWith(makeValuesBinding, vardecls, values)))
        (mapResult((sexpArr: Sexp[]) => mapResult(parseVarDecl,sexpArr), map(b => b[0], bindings)), mapResult(parseL5CExp, map(b => b[1], bindings)));

const parseLetValuesExp = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> =>
    isEmpty(body) ? makeFailure('Body cant be empty') :
    !isGoodValuesBindings(bindings) ? makeFailure(`Bindings are not good`) :
    safe2((binds: ValuesBindings[], body: CExp[]) => makeOk(makeLetValuesExp(binds, body)))
...
const unparseVarDecls = (vardecls: VarDecl[]): Result<string> =>
    bind(mapResult(unparseVarDecl, vardecls), parsedArr => makeOk(`(${parsedArr.join(' ')})`));

const unparseValuesBindings = (bindings: ValuesBindings[]): Result<string> =>
    bind(mapResult(bind => safe2((vardecls: string, value: string) => makeOk(`(${vardecls} ${value})`))
        (unparseVarDecls(bind.vardecls), unparse(bind.value)), bindings),
        (binds: string[]) => makeOk(join(" ", binds)));
    
const unparseLetValuesExp = (letvalue: LetValuesExp) : Result<string> => 
    safe2((binds: string, body: string) => makeOk(`(let-values (${binds}) ${body})`))
        (unparseValuesBindings(letvalue.bindings), unparseLExps(letvalue.body));