...
export const parseTExp = (texp: Sexp): Result<TExp> =>
    ...
    isArray(texp) && texp.indexOf('->') === -1 ? parseSingleTuple(texp) :
    makeFailure(`Unexpected TExp - ${texp}`);


const parseSingleTuple = (texparr: Sexp[]): Result<TupleTExp> =>
    bind(parseTupleTExp(texparr),
        (tuple: TExp[]) =>
            isEmpty(tuple) ? makeOk(makeEmptyTupleTExp()) :
            allT(isNonTupleTExp , tuple) ? makeOk(makeNonEmptyTupleTExp(tuple)) :
            makeFailure(`No nested tuples are allowed!`));
